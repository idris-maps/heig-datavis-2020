...
export const getXByYear = year => d =>
  xScale(d.data.find(([first]) => first === year)[1])
export const getYByYear = year => d =>
  yScale(d.data.find(([first]) => first === year)[2])
export const getRByYear = year => d =>
  rScale(d.data.find(([first]) => first === year)[3])
export const getColor = getColorByRegion